...
private fun getCircleColor(@IntRange(from = RANDOM_COLOR_START_RANGE.toLong(), to = RANDOM_COLOR_END_RANGE.toLong())
    ...
    val colorId = AppCastingAgency.getInstance().resources.getIdentifier(colorIdName, "color", AppCastingAgency.getInstance().packageName)
    return AppCastingAgency.getInstance().resources.getColor(colorId)
}

fun getGreyCircleDrawable(): Drawable {
    return getColoredCircleDrawable(AppCastingAgency.getInstance().resources.getColor(R.color.grey_dark))
}

private fun getColoredCircleDrawable(@ColorInt color: Int): Drawable {
    val drawable = AppCastingAgency.getInstance().resources.getDrawable(R.drawable.shape_circle) as GradientDrawable
