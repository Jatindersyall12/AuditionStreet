...
import com.auditionstreet.castingagency.ui.chat.adapter.DialogsAdapter
import com.auditionstreet.castingagency.ui.chat.adapter.manager.DialogsManager
import com.auditionstreet.artist.ui.home.activity.HomeActivity
import com.auditionstreet.artist.ui.home.fragment.EXTRA_QB_USERS
import com.auditionstreet.castingagency.utils.chat.*
import com.auditionstreet.castingagency.utils.chat.callBacks.QBPushSubscribeListenerImpl
import com.auditionstreet.castingagency.utils.chat.callBacks.QbEntityCallbackImpl
import com.auditionstreet.artist.utils.showToast
import com.auditionstreet.castingagency.async.BaseAsyncTask
...
class DialogsActivity : BaseActivity(), DialogsManager.ManagingDialogsCallbacks {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setUpToolbar(toolbar,getString(R.string.chat),false,false)
        if (intent.getSerializableExtra(EXTRA_QB_USERS) != null) {
            val selectedUsers = intent.getSerializableExtra(EXTRA_QB_USERS) as QBUser
            getUserFromAppAndCreateDialog(selectedUsers)
        }
    ...
    private fun checkPlayServicesAvailable() {
        ...
        if (resultCode != ConnectionResult.SUCCESS) {
            if (apiAvailability.isUserResolvableError(resultCode)) {
                ...
                showToast(this, "This device is not supported")
                finish()
            }
        }
    }
   ...
   /* private fun userLogout() {
    ...
    private fun initUi() {
                ...
                ChatHelper.loginToChat(currentUser,
                        object : QBEntityCallback<Void> {
                          ...
                          //      hideProgressDialog()
                            ...
                            override fun onError(e: QBResponseException?) {
                            //    hideProgressDialog()
                                showToast(this@DialogsActivity, resources.getString(R.string.login_chat_login_error))
                            }
                        })
            }
        }
        ...
        refreshLayout.setColorSchemeResources(R.color.orange, R.color.grey_dark, R.color.red, R.color.gray)
    }
    ...
    private fun loadDialogsFromQb(silentUpdate: Boolean, clearDialogHolder: Boolean) {
        ...
        ChatHelper.getDialogs(requestBuilder, object : QBEntityCallback<ArrayList<QBChatDialog>> {
            ...
            override fun onError(e: QBResponseException) {
                disableProgress()
                showToast(this@DialogsActivity, e.message!!)
            }
        })
    }
        ...
        private fun deleteSelected() {
            ...
            for (dialog in selectedDialogs) {
                if (dialog.type == QBDialogType.PUBLIC_GROUP) {
                    showToast(this@DialogsActivity, getString(R.string.dialogs_cannot_delete_chat, dialog.name))
    ...
    private class DialogJoinerAsyncTask internal constructor(dialogsActivity: DialogsActivity,
        ...
        override fun onException(e: Exception) {
            ...
            if (!isCancelled) {
                ...
                //showToast(this@DialogActivity,"Error: " + e.message)
            }
        }
    ...
    private fun getUserFromAppAndCreateDialog(selectedUser: QBUser){
        if (isPrivateDialogExist(selectedUser)) {
            val existingPrivateDialog = QbDialogHolder.getPrivateDialogWithUser(selectedUser)
            isProcessingResultInProgress = false
            existingPrivateDialog?.let {
                ChatActivity.startForResult(this, REQUEST_DIALOG_ID_FOR_UPDATE, it)
            }
    ...
    private fun createDialog(selectedUsers: QBUser) {
        ChatHelper.createDialogWithSelectedUsers(selectedUsers, "",
            object : QBEntityCallback<QBChatDialog> {
                override fun onSuccess(dialog: QBChatDialog, args: Bundle?) {
                    Log.d(TAG, "Creating Dialog Successful")
                    isProcessingResultInProgress = false
